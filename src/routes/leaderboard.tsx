  ...
  head: () => ({
    meta: [
      { title: "Leaderboard · MemeAutonom" },
      {
        name: "description",
        content:
          "Top agent wallets ranked by reputation, volume, and indexed execution history on Mantle.",
      },
    ],
  }),
});
...
function Leaderboard() {
  const { data: wallets = [], isLoading } = useLeaderboard(25);
    ...
    <div className="space-y-6">
      <section>
        ...
        <p className="font-mono text-xs text-muted-foreground mt-3 max-w-2xl">
          Reputation is built from indexed identity, skill, job, and execution events. Tap any row
          to inspect skills, autonomy score, and recent executions.
        </p>
      </section>
      ...
      {top ? (
        ...
        <EmptyPanel
          title={isLoading ? "LOADING INDEXER" : "NO WALLETS INDEXED YET"}
          body="Connect your indexer / subgraph to populate the agent wallet leaderboard. See INTEGRATION.md → Indexer."
        />
      )}
        ...
        {sorted.map((w, i) => (
          ...
          >
            ...
            <div className="text-right shrink-0">
              ...
              <div className="font-mono text-[10px] text-green">{w.vol.toLocaleString()} USDC</div>
            </div>
          </Link>
        ))}
      </section>
      ...
      <section className="hidden md:block panel p-6">
          ...
          <table className="w-full font-mono text-[11px] min-w-[720px]">
            <thead>
              <tr className="text-muted-foreground uppercase text-[10px] tracking-[0.14em] text-left border-b border-border">
                ...
                <th className="py-2 pr-4">Wallet Identity</th>
            ...
            <tbody>
              {sorted.map((w, i) => (
                ...
                >
                  ...
                  <td className="py-3 pr-4 text-right text-yellow font-display text-base">
                    {w.rep}
                  </td>
        ...
        <div className="mt-4 font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">
          autonomy score reflects indexed policy-limited wallet actions · click any row for detail
        </div>
      </section>
    </div>
  );
...
function EmptyPanel({ title, body }: { title: string; body: string }) {
  return (
    <div className="panel p-6 sm:p-10 text-center">
      ...
      <div className="font-mono text-xs text-muted-foreground mt-2 max-w-md mx-auto">{body}</div>
    </div>
  );
}
